...
### 1. Page Layout (页面布局)
- **Container Width**: 页面主容器宽度强制为屏幕的 **100%** (`width: 100%`)，撑满容器（配合 `box-sizing: border-box`），不再缩在中间。
- **Overflow**: 内容（图表/画布）超出屏幕时，容器内 `overflow-x: auto` / `overflow-y: auto` 支持上下左右滚动。
...
页眉采用统一精简的左右分布布局，高度建议控制在 `56px - 90px`（不占高空间），全站保持一致。
- **容器样式**: 背景 `#FFFFFF`，底部 `1px #E2E8F0` 分割线，无圆角/阴影。
- **Left Side**:
    - **Title (H1)**: 企业或项目的主标题，使用 `Starry Blues (#10213E)`，加粗，字号 `22px`。**标题前统一加 4px 宽的 `Creative Blue (#5DB2E2)` 竖线装饰**（`::before`），若配置图标则图标统一为 `20px`、同色。
    - **Subtitle**: 业务类型或副标题，使用 `Muted Text (#64748B)`，字号 `13px`，左缩进与竖线对齐。
- **Right Side**:
    - **Actions**: 「导出 PDF / 打印」「复制原始 YAML」等操作按钮，统一字号 `12px`、圆角 `6px`、内边距 `6px 12px`。
    - **Button 样式**: 次要按钮（导出）白底 + `#E2E8F0` 描边 + `#10213E` 文字；主要按钮（复制 YAML）`#10213E` 底 + 白字。
    - **标签/徽章 (可选)**: 字号 `12px`、`#F5F5F6` 底、`#64748B` 文字、圆角胶囊 `10px`。
- **布局**: `display: flex; justify-content: space-between; align-items: center;` 左右分布，必要时 `flex-wrap: wrap`。

### 3. File & Directory Management (文件与目录管理)
- **Directory（两层分层）**: 所有输出存放于以**公司/业务名**命名的子目录中；属于**具体 AI 场景**的产物（AI Canvas / To-be 旅程 / CKD / 故事 / MVP 原型）必须再放入 **`<公司/业务名>/<场景名>/`** 场景子目录，工程产物固定置于 `<公司/业务名>/<场景名>/mvp-prototype/`（例如：`张雪机车海外销售/售后理赔/`、`张雪机车海外销售/售后理赔/mvp-prototype/`）。
- **Filename**: 公司级产物命名 `{公司名}-{内容类型}.html`；场景级产物命名 `{公司名}-{场景名}-{内容类型}.html`（例如：`张雪机车-OSM业务战略地图.html`、`张雪机车-售后理赔-To-be旅程.html`）。

## Pre-Delivery Checklist
- [ ] 容器宽度是否撑满（width: 100% + box-sizing: border-box）
- [ ] 页眉是否采用统一精简样式（左侧标题/副标题、右侧操作按钮，高度 ≤90px）
- [ ] 图表/画布超出屏幕时是否可在容器内上下左右滚动
- [ ] 字体是否强制使用 MiSans (优先)
- [ ] 输出路径是否遵循两层规范：场景级产物在 `{公司名}/{场景名}/` 下、命名 `{公司名}-{场景名}-{内容类型}.html`；工程产物在 `<公司名>/<场景名>/mvp-prototype/`（若产物属公司/业务级则放 `{公司名}/` 根目录）
