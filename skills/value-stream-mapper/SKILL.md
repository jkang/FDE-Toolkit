...
description: |
  梳理某个业务端到端的 L1 级价值流，输出「价值流图」（对齐 03-业务价值流图.jpg 风格），
  并据「原始 idea 需求」在全链路上标注「聚焦范围」——同一价值段内相邻(连续)的聚焦业务环节
  整体包成一个「聚焦框」（★ 标签 + 卡级优先级）。
...
> [!IMPORTANT] 业务纯净性 + 命名一致性（本 Skill 的第一纪律）
> L1 价值流图是**业务地图**，必须**与解决方案解耦、完全站在业务视角**。除 `meta.originalIdea`
> （客户原话，仅作聚焦判定依据）外，所有业务内容字段一律**不得出现**：
> 智能×× / 智慧×× / AI / 大模型 / Agent / 机器人 / "自动××"（表系统代劳）/ 具体系统·产品·工具名。
> 价值段名与业务环节名统一采用 **〔部门/岗位 ·〕业务对象(名词) + 业务动作(动词)** 结构
> （如 `客服 · 订舱请求受理与校验`、`货主 · 运价查询与比价`），岗位词典全图一致。
> 完整规则见 `references/value_stream_prompts.md` §2；编译时 `build_value_stream.py` 会做防呆告警。
...
> - **文件名**: `[公司/业务名]-价值流图.html`；YAML 同理 `[公司/业务名]-价值流图.yaml`
>   （例如：`X电商订舱-价值流图.html`）。
...
>   同列内**相邻聚焦环节整体包成一个聚焦框**（★标签 + 卡级优先级）+ 底部图例。
...
2. **Compiler**: `scripts/build_value_stream.py` 解析 YAML，结合 `templates/value_stream_layout.html`
   生成价值链总览图；自动完成**聚焦范围分组与连续编号（价值段内相邻聚焦环节聚成一框）**、
   **统计**、**防呆清洗**。
...
| **valueStreams** | 横向一条 = 一条端到端主线；含 chain 副标题 | 价值流概述条带（①序号+名称+价值段串联小字） |
| **segments** | 列 = 一个价值段；name + chain（业务环节串联）+ links（业务环节卡）+ items（渲染序列：普通卡/聚焦框交错） | 横向从左到右串联列（→ 连接） |
| **links** | 业务环节卡 = 价值段内的子环节；focus / priority / detail | 列内竖排卡（↓ 连接） |
| **聚焦框 (focusGroup)** | 同一价值段内**相邻(连续)的聚焦环节**自动聚成一组；label + 组内 links | 列内蓝描边大框 + 顶部「★ 聚焦范围 ①」 |

价值段（segment，= 横向列）核心字段：

| 字段 | 含义 | 取值/示例 |
|------|------|-----------|
| `name` | 价值段名 | `询价与报价`（对象+动作；协作段可省岗位前缀） |
| `chain` | 列头副标题（业务环节串联） | `货主 · 运价查询与比价 → 客服 · 锁价申请受理与确认` |
...
| 字段 | 含义 | 取值/示例 |
|------|------|-----------|
| `name` | 业务环节名（「主体 · 对象+动作」结构） | `客服 · 锁价申请受理与确认` |
| `focus` | 是否聚焦 | `true` / `false` |
| `focusLabel` | 聚焦框标签 | `聚焦范围 ①`（可省略，编译器把同一价值段内连续聚焦环节聚成一框并全局递增编号） |
| `priority` | 优先级 | `P0` / `P1` / 空串 |
| `detail.definition` | 一句话定义 | `货主查询即期与长约运价并多方案比价` |
| `detail.goal` | 业务目标 | `缩短报价与锁价确认周期、提升成交转化` |
| `detail.reason` | 聚焦/不聚焦理由 | `聚焦理由：…` 或 `不聚焦理由：…` |

聚焦框（focusGroup，= 列内蓝框，编译器自动生成）核心字段：

| 字段 | 含义 | 取值/示例 |
|------|------|-----------|
| `label` | 聚焦范围标签 | `聚焦范围 ①`（全局递增，跨段不合并编号） |
| `links` | 组内聚焦业务环节 | `[ {name, priority, detail}, ... ]`（同一价值段内相邻连续聚焦环节） |
...
- **产物保存到 `<公司/业务名>/` 根目录**，命名 `[公司/业务名]-价值流图.yaml`
  （`examples/` 仅存放演示样例）。
...
- [ ] **纯业务语言**：除 `meta.originalIdea` 外，全文无 智能/AI/Agent/大模型/机器人/"自动××"（表系统代劳）/具体系统·产品·工具名；编译无防呆告警（或告警已人工复核）
- [ ] **命名一致**：价值段名与业务环节名均为「〔岗位 ·〕对象+动作」结构，岗位词典全图一致（不换词）
- [ ] 聚焦范围以「价值段」为边界：同一价值段内**相邻(连续)的聚焦环节**聚成一个**聚焦框**，不跨价值段合并
- [ ] 聚焦环节尽量在同一价值段内**密集连续**（对应参考图「一个聚焦框包住多个环节」，如参考图②列一个框包 3 个环节）
- [ ] 聚焦框自动递增编号 ① ② ③…（全局按阅读顺序，跨段不合并；同段被非聚焦环节隔开的多个框各自递增）
- [ ] 聚焦框内每个环节卡有 name / priority / detail，`focus: true` 且 reason 写「聚焦理由：…」
- [ ] 聚焦/优先级不设在价值段（列）上；聚焦框只包段内聚焦环节，非聚焦环节为独立普通卡（顺序交错）
