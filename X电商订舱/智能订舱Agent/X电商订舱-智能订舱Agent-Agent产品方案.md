...
**重点看什么：** 核心仍是「在线订舱工作台」——它是智能订舱 Agent 多能力编排的交付载体（Artifact），全场景唯一 P0（对应任务流程拆解 S3「在线订舱 · 一键订舱与确认」，L5 最密集处）。本版的关键升级在两点：① 故事线 mock 数据从画布口径，对齐到 SKP 已深挖的决策因子矩阵（报价解读因子、舱位综合排序因子、订舱校验规则引擎 200+ 条、单证规则库 300+ 条、ETA 预测多源因子）；② 「五类规则消费门禁」用任务处理规则挖掘清单 + 本体 action_boundaries 的真实承接——把「报价/评分必须来自确定性服务」「订舱为对外承诺必须人工放行」「SI 缺件禁止推送」等护栏，逐一落到 HITL 拦截与出处引用。
...
**当前洞察：** SKP 与本体揭示的规则结构高度统一——决策模型类经验集中在「报价解读 / 舱位排序 / 订舱校验 / 单证校验 / ETA 预测」五个环节，且集中在订舱执行与单证预填两段；其中「对外放行订舱」「异常升级人工终审」「SI 缺件禁止推送」「HS 无法映射禁止猜」是硬约束（不可突破），「成本优先」「货期权衡」是软优先级（可冲突仲裁）。这意味着 HITL 设计只需覆盖两类硬闸门 + 少量软优先级冲突点，不必事无巨细地拦截。

**需要 FDE 做什么：** ① 核对本版故事线 mock 数据与 SKP 决策阈值的一致性（订舱校验规则引擎 200+ 条的分类口径、ETA 预测 68% 概率的触发条件）；② 确认「舱位保证 Space Protection 兑现率 95%+」「单证一次提交通过率」的度量口径；③ 视觉基底 P0/P1/P2/P3 锚定与相邻系统（报价引擎 / 舱位收益系统 / OMS / 控制塔 / 智慧单证 / 财务）边界，属阶段2 原型的前置项。

## 2 产品定位

### 2.1 迭代范围

**功能迭代优化（SKP + 本体精化）**：在 v1.0 概念级方案基础上，用已完成的 SKP「深度任务序列（21 个 L5）+ 任务处理规则挖掘清单（五类规则）」与本体「三层本体（对象关系 / 行动边界 / 状态迁移）」回填精化。首版 PoC 范围不变：电商订舱主链路「询价 → 舱位推荐 → 一键订舱 → 单证预填 → 出运跟踪」，聚焦在线订舱一步（P0），验证「AI 问答 → 可执行订舱」能否在订舱确认时效、AI 分流率、单证一次提交通过率之间取得平衡。
...
> 共 4 条，1:1 对应阶段2 `moments/*.html`。mock 数据均取自 SKP「任务处理规则挖掘清单」与本体 action_boundaries 的真实决策因子/阈值（脱敏）。
...
| 业务内容来源 | SKP 深度任务·自然语言发起询价/意图识别与路由/实时报价数据返回/报价逐项解读；SKP 决策模型类·报价解读经验；本体 ab_quote_recommend（报价/评分必须来自确定性服务）；CKD·报价引擎/舱位收益/FAQs/运价术语字典 |
...
| 业务内容来源 | SKP 深度任务·舱位余量/船期数据返回/综合排序推荐 Top3/解释推荐理由/确认 Agent 推荐舱位/3 步表单自动预填/订舱规则校验/一键订舱确认/签发 Booking Confirmation/异常订舱人工终审；SKP 决策模型类·舱位排序经验 + 订舱校验规则引擎；本体 ab_booking_submit（订舱为对外承诺必须货主人工放行（gate）） |
...
| 业务内容来源 | SKP 深度任务·上传贸易单据/OCR 抽取 SI 字段/品名规范与 HS 映射/单证规则校验/确认并提交 SI；SKP 关键信息提取要点·PI/装箱单字段典型位置与易错变体；本体 ab_si_document（SI 缺件禁止推送、HS 无法映射禁止擅自猜测）；CKD·智慧单证规则库 >300 条 |
...
| 业务内容来源 | SKP 深度任务·对话查里程碑/ETA 预测与延误预警；SKP 决策模型类·ETA 预测 + 改配推荐经验；本体 ab_tracking_after_sales（ETA 置信度低→降级事后通知、D&D 各港不同→口径不一致标注待人工）；CKD·港口拥堵/ETA 预测模型、D&D 规则库 |

## 5 业务功能与 UI 组件清单

| 功能名 | 一句话价值 | 绑定角色 | 渲染层 | 触发方式 | UI 组件名 + 组件内容 | 用户操作 | 关联故事线 | 上游来源 |
|---|---|---|---|---|---|---|---|---|
| 对话询价与报价解读 | 一句口语变逐项报价卡 | SME 货主 | Conversation + Artifact Pane | 用户点击召唤 | 报价解读卡（报价/有效期/费用构成/市场基准/舱位状态） | 只读 | 故事线1 | SKP 深度任务·报价逐项解读；SKP 决策模型类·报价解读经验；本体 ab_quote_recommend |
...
| 一键订舱与确认 | 预填+校验+人工放行 | SME 货主 | Artifact Pane | Agent 主动渲染 | 在线订舱工作台（12 字段预填 + 校验结果 + 人工放行 HITL） | 只读/可编辑/可提交 | 故事线2 | SKP 深度任务·3 步表单自动预填/订舱规则校验/一键订舱确认；SKP 模版范例类·订舱表单模板；本体 ab_booking_submit |
| 单证预填与校验 | 上传即预填 SI + 缺件提醒 | SME 货主 | Artifact Pane | Agent 主动渲染 | SI 预填校验表（字段/识别值/校验状态）+ 缺件示例卡 | 只读/可补件 | 故事线3 | SKP 深度任务·OCR 抽取 SI 字段/单证规则校验；SKP 关键信息提取要点；本体 ab_si_document |
...
| 在线支付与售后 | 多币种支付 + 会员权益 | SME 货主 | Artifact Pane | 用户点击召唤 | 支付结果卡（金额/币种/发票号/系统同步）+ 会员权益条 | 只读/可支付 | 故事线4 | SKP 关联关系类·货主-订舱-费用关联；本体 ab_tracking_after_sales |
| 术语口径卡 | 悬停看懂专业口径 | 全体 | Conversation Pane | 自动激活 | 术语 hover 卡（E-Spot/Secured E-Quote/E-Quote/BAF/THC/D&D/Space Protection/VGM/HS 定义） | 只读 | 故事线1/2/3 | SKP 术语字典类·航运电商口径词典；CKD·运价术语字典/FAQ 知识库 |
| 出处引用 | 决策依据可追溯 | 全体 | Conversation Pane | Agent 主动渲染 | 出处引用悬浮卡（字段典型位置 + 决策因子确定性 + 数据源） | 只读 | 故事线1/2/3 | SKP 关键信息提取要点·字段典型位置；SKP 决策模型类·决策因子确定性；CKD·数据源 status |
...
| 使用了哪些数据/知识 | 始终标注来源 | SKP 术语字典类/关键信息提取要点/CKD 数据源 → #7 出处引用 |
| Agent 不确定/置信度低 | 显式标注 | SKP 决策模型类·决策因子确定性、OCR 置信度 → #5 HITL + #7 |
...
| 修改关键业务数据 | 订舱/SI 字段异常（负值/超范围/信用受限/缺件/毛重超限） | 写入前必停 | SKP 决策模型类·订舱校验规则引擎 + 智慧单证规则库（>300 条）；本体异常触发 |
| Agent 自判不确定 | 报价歧义/舱位排序边界冲突/ETA 多源数据缺失/HS 无法映射 | 提示用户决策 | SKP 决策模型类·报价/舱位/ETA 预测；本体 exception_triggers |
